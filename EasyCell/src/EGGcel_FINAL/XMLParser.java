	...
	public static void writeXML(Model t, String destination) {
		...
		try {
			...
			for (int row = 0; row < t.getRows(); row++) {
				for (int col = 0; col < t.getCols(row); col++){	
                                    if(!((t.cellReader(t.getContent(row, col)))[0].equals(""))){
                                    Element cell = xmldoc.createElement("CELL");
					// set attributes
					cell.setAttribute("column", Integer.toString(col + 1));
					cell.setAttribute("row", Integer.toString(row + 1));
					// set content
					cell.appendChild(xmldoc.createTextNode((String) (t.cellReader(t.getContent(row, col)))[0]));
